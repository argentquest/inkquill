...
AI_TEMPERATURE_SETTINGS: Dict[str, float] = {
    ...
    "context_conversion": 0.3,       # Consistent Context text conversion
...
AI_MAX_TOKEN_SETTINGS: Dict[str, int] = {
    ...
    "context_conversion": 700,       # Short Context descriptions
...
AI_TOP_P_SETTINGS: Dict[str, float] = {
    ...
    "context_conversion": 0.9,       # Rich descriptive text
...
### 4. Context and Processing Settings

```python
Context_RETRIEVAL_TOP_K: int = Field(default=3, ...)  # Number of documents to retrieve
...
async def retrieve_context_context(self, query: str, top_k: int = None):
    if top_k is None:
        top_k = settings.Context_RETRIEVAL_TOP_K
    # ... retrieval logic
```
...
- **context_conversion**: Low max tokens (700), low temperature (0.3), high top_p (0.9)
...
- Context top_k is between 1 and 20
...
4. **Processing and Context**
   - `app/services/context_retrieval.py`: Configurable top_k
...
- 25 max token limits  
- 15 top_p values
- 5 Context/processing constants
