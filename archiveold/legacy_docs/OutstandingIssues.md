...
4.  **Task ID: `P8_03_ENHANCE_Context_SEMANTIC_SEARCH`**
    *   **Task:** **Implement Semantic Ranking for Context.**
...
3. the quality of Context results by implementing hybrid search with semantic ranking. This involves modifying the Azure AI Search query in `app/services/context_retrieval.py` to use `query_type="semantic"` and the pre-defined `semantic_configuration_name`.  **Task ID: `P7_01_CLEANUP_OBSOLETE_JS`**
...
These are the next major
    ...
    *   **Files Affected:** `app/services/context_retrieval.py`.
...
4.  **Task ID: `P8_03_ENHANCE_Context_SEMANTIC_SEARCH`**
    *   **Description:** Implement and utilize semantic search (or hybrid search with semantic ranking) for Context retrieval. This involves modifying the search queries in `app/services/context_retrieval.py` to use `query_type="semantic"` and a `semantic_configuration_name narrative generation. Instead of just a simple context string, consider passing a more structured summary of traits, motivations, and relationships to make the AI more "character-aware."
...
8.  **Task ID: `P4_02_FEATURE_Context_PREVIEW_EDITORS`**
    *   **Description:** In the Act/Scene editors, display a preview of the Context context snippets *before* the main AI narrative generation begins. This requires the WebSocket to send a dedicated message with the Context results first.
    *   **Status:** New Feature.
...
*   **Task ID: `P0_02_WEBSOCKET_ACT_EDITOR (`azure_blob_service.py`) and a new Context ingestion module (`context_ingestion.py`).
