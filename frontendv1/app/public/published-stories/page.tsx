...
import { fetchPublishedStories, type PublishedStoryEntry } from "@/lib/api";

function PublishedStoryCard({ story }: { story: PublishedStoryEntry }) {
  return (
    <Link href={`/public/stories/${story.id}`}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-ink-500">
              {story.publisher_display_name ? (
                <span>by {story.publisher_display_name}</span>
              ) : null}
              {story.world_name ? (
...
export default function PublishedStoriesPage() {
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: () => fetchPublishedStories({ sort_by: "recent", per_page: 40 }),
  });
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        description="Stories published by writers on the platform. Read, rate, and discover new worlds."
        eyebrow="Published Stories"
      ...
      {isLoading ? (
        ...
        <section
          ...
          data-testid="published-stories-empty-state"
        >
          ...
          <p className="mt-2 max-w-sm text-sm text-ink-500">
            Writers who publish their work will appear here. Check back soon.
          </p>
        </section>
      ) : (
        <section className="space-y-4" data-testid="published-stories-list">
