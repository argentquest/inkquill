...
Configuration Variables for Storage: Added AZURE_STORAGE_CONNECTION_STRING and AZURE_STORAGE_ACCOUNT_NAME to the configuration loading section, with appropriate placeholder defaults.
Python

# Azure Storage Configuration
AZURE_STORAGE_CONNECTION_STRING = os.getenv("AZURE_STORAGE_CONNECTION_STRING", "STORAGE_CONNECTION_STRING_NOT_SET")
AZURE_STORAGE_ACCOUNT_NAME = os.getenv("AZURE_STORAGE_ACCOUNT_NAME", "STORAGE_ACCOUNT_NAME_NOT_SET")
...
The connection string (if AZURE_STORAGE_CONNECTION_STRING is set and not a placeholder).
The account name with DefaultAzureCredential() (if AZURE_STORAGE_ACCOUNT_NAME is set and not a placeholder, and connection string isn't). This allows testing Managed Identity scenarios if you're running this script from an Azure environment with MI configured, or using local Azure CLI login.
...
# AZURE_STORAGE_ACCOUNT_NAME="yourstorageaccount" # Optional if using connection string
Option 2 (Account Name for Managed Identity / DefaultAzureCredential):
Code snippet

# AZURE_STORAGE_CONNECTION_STRING="" # Leave blank or omit if using MI
...
Move on to implementing the actual application code, like the document_processor.py for the RAG pipeline.
