...
Blob Storage: Must use the correct AZURE_STORAGE_CONNECTION_STRING (or AZURE_STORAGE_ACCOUNT_NAME if using Managed Identity) and AZURE_STORAGE_CONTAINER_NAME_FOR_RAG_DOCS (from .env: "stories").
...
RAG Retrieval (app/services/rag_retrieval.py - RetrievalPlugin):
...
app/services/rag_retrieval.py: Ensure it uses settings for OpenAI Embeddings and AI Search.
...
Services (app/services/): Log calls to external services (Semantic Kernel, Azure SDKs), parameters being sent (mask secrets), and summaries of responses. For example, in rag_retrieval.py: logger.info(f"RAG query: '{query}', retrieved {num_chunks} chunks.").
...
AZURE_STORAGE_CONTAINER_NAME_FOR_RAG_DOCS (ensure this is "stories")
