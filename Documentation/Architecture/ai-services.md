...
- [AI Search and RAG Implementation](#ai-search-and-rag-implementation)
...
graph TB
    ...
    class AI_SEARCH_INDEX,POSTGRES,BLOB_STORAGE storage
```
...
6. **RAG Retrieval Plugin**: Context-aware content retrieval
...
## AI Search and RAG Implementation
...
sequenceDiagram
    participant User
    participant App as Application
    participant RAG as RAG Plugin
    participant Search as AI Search
    participant LLM as Language Model
    
    User->>App: Content generation request
    App->>RAG: retrieve_rag_context(query)
    RAG->>Search: Vector search for relevant content
    Search-->>RAG: Relevant documents
    RAG->>RAG: Format context for prompt
    RAG->>LLM: Generate with context
    LLM-->>RAG: Generated content
    RAG-->>App: Enhanced response
...
RAG_RETRIEVAL_TOP_K = 3  # Number of documents to retrieve
RAG_WRITING_ASSISTANT_TOP_K = 10  # For writing assistance
RAG_CHAT_TOP_K = 7  # For chat interactions
