"""add nature_park provider to care_circle_provider_catalog
...
def upgrade() -> None:
    op.execute("""
        ...
        VALUES
            ('nature_park', 'National Park', '🏞️', 'memory', true, 42, true, true, 'daily_newsletter')
...
def downgrade() -> None:
    op.execute(
        "DELETE FROM care_circle_provider_catalog WHERE provider_key = 'nature_park'"
    )
