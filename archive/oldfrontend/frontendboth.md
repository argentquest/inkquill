# Frontend System Documentation — Reconciled Reference

> **Purpose:** This document is the authoritative, reconciled reference for the InkandQuill2 application frontend. It merges `frontend.md` (high-level capability overview) and `frontendclaude.md` (detailed technical reference) into a single source of truth. It serves as the specification for the React frontend rewrite. Every page, component, interaction pattern, and API contract documented here must be replicated or intentionally superseded in the new frontend.
>
> **Reconciliation notes:** Where the two source documents disagreed, the more technically accurate/specific version was preferred and noted. Key corrections: the backend is **FastAPI** (not Flask); the primary HTTP client is the **Fetch API** (not jQuery); jQuery is not used.
...
> **Correction from frontend.md:** The backend framework is **FastAPI** (not Flask). Both use Jinja2 for templating, but the runtime, routing, and async model differ significantly. jQuery is **not** in the stack; all HTTP requests use the native Fetch API.

| Concern | Current Technology | Notes |
|---|---|---|
| Backend Framework | FastAPI with Jinja2 (server-side rendered) | Async Python; not Flask |
| UI Framework | Tabler Premium (Bootstrap-based) | Custom overrides via CSS variables |
| Icons | Font Awesome 6.5.2 | CDN-loaded |
| Rich Text Editor | Quill.js 1.3.6 | CDN-loaded |
| Real-time | Native WebSocket API | Ticket-based auth |
| HTTP Requests | Fetch API with `credentials: 'include'` | No jQuery |
| Theme | CSS custom properties + localStorage | Prevents FOUC via early-detection script |
| Maps | Custom canvas/SVG world map | `world_map.js` |
| Charts | Chart.js | Data visualization on dashboards |
| Auth | JWT in HttpOnly cookies | OAuth (Google) also supported |
| Image Generation | DALL-E 3, RunPod | Selectable per generation |
| Payments | Stripe | Subscription and one-time purchases |
...
- CSRF protection via token-based request validation
- Rate limiting: API call restrictions and abuse prevention
- Input sanitization: XSS prevention via `escapeHtml()` in `utils.js`
...
- Acts list with expand/collapse; scenes nested under acts
...
- **Export:** PDF export supported (`pdf_export_service.py`). EPUB/plain text export is planned but not confirmed implemented.
...
- Steps: World selection → Story concept → Genre/tone → AI outline generation → Review & confirm
...
- Fields: name, description, genre, visibility (public/private), cover image, color scheme
...
- Cards for published stories with thumbnail, title, author, stats, description
- Search and filter by genre, author, tags, popularity
- Sorting: Newest, most viewed, highest rated
- Pagination
...
- Profile info: avatar, bio, preferences
- Coin balance
- Subscription info
- Password change
...
- Reward status (coins for successful referrals)
...
- Job polling: `image_job_monitor.js` (polls every 2s until complete)
...
- All layouts use `container-fluid` (full width) per CLAUDE.md standards
...
### Key UI Patterns (from CLAUDE.md Design Standards)

- **Hero headers (REQUIRED on every page):** `linear-gradient(135deg, var(--tblr-primary) 0%, var(--tblr-purple) 100%)` with radial overlay
...
- **Buttons on gradient backgrounds:** `btn btn-light`; help buttons: `btn btn-warning`
...
- World context (via direct context assembly)

**AI Providers supported:**
- OpenAI, Anthropic, Google, and others (configured in AI Models admin)
- Dynamic model switching based on task
- Token tracking, usage monitoring, and cost calculation per operation
...
### 11.7 Publishing System

- Convert drafts to published stories
- Published stories visible in the public gallery
- Comments, likes, and bookmarks on published stories
- Social sharing via Open Graph meta tags
...
- Skeleton loading placeholders where implemented
...
### Payment Processing
- **Stripe** integration for subscription and one-time coin purchases
- Transaction logging with detailed payment records
- Virtual coin system: coins purchased via Stripe, then spent on AI features
...
- Critical content loaded first, enhancements later
- Page-specific JS loaded only on pages that need it (no bundler, no unused code)

### Accessibility
- ARIA labels on interactive elements
- Keyboard navigation support
- Tooltips required on all interactive elements (per CLAUDE.md)
- Descriptive button text required — icon-only buttons prohibited

### Known Technical Debt
- No JavaScript bundler — all scripts are individual `<script>` tags
- No component framework — moving to React is the planned solution
- Some legacy patterns in older pages that pre-date design system
...
- Dynamic model switching based on tasks via AI Model Configurations
- Semantic Kernel (Microsoft) for AI orchestration on the backend
- Token tracking, usage monitoring, and cost calculation
- Image generation: DALL-E 3 (OpenAI) and RunPod

### Payment Processing
- **Stripe**: Subscription and one-time coin purchases

### Analytics
- Google Analytics 4: User behavior tracking, custom events

### Social Media
- Open Graph meta tags for rich social previews
- Twitter Card support
- Sharing integration: Facebook, Twitter, LinkedIn
...
The primary planned enhancement is a full React rewrite replacing Jinja2 server-rendered templates. This document serves as the specification for that rewrite. Key considerations:

- **SPA Routing:** Replace server-side routes with React Router
- **State Management:** Replace module-level variables + DOM state with a proper store (Redux, Zustand, or React Query)
- **Component Library:** Replace Tabler Jinja2 partials with Tabler React components
- **WebSocket:** Wrap existing WebSocket protocol in custom React hooks
- **Rich Text:** Replace Quill.js 1.3.6 with a React-compatible editor (TipTap recommended as Quill successor)
- **Authentication:** JWT cookie auth remains unchanged; wrap in React context
- **API Layer:** Existing `/api/v1/` contracts remain unchanged — only the frontend consuming them changes

### Features Currently Planned / Partially Implemented
> These appeared in `frontend.md` as current features but are not confirmed fully implemented in the codebase:

- **EPUB/plain text export:** PDF export is confirmed implemented (`pdf_export_service.py`). EPUB export should be verified before including in React scope.
- **Offline support:** Not confirmed implemented. Would require PWA service worker work.
- **Version control for stories:** Git-style change tracking is noted as a goal but not confirmed in current implementation.
- **Multi-user story collaboration:** World collaborators exist (`world_collaborator` model). Story-level collaboration scope should be verified.

### Progressive Web App
- Offline functionality and app-like experience are longer-term goals
...
Transactional emails sent by the backend — not frontend pages. Templates in `app/templates/emails/`:
...
*This document was reconciled from `frontend.md` and `frontendclaude.md` as of 2026-03-30. It serves as the authoritative specification for the React frontend rewrite.*
