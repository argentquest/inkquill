# /ai_rag_story_app/app/core/config.py
import os
import logging
...
class Settings(BaseSettings):
    ...
    AZURE_OPENAI_ENDPOINT: Optional[AnyHttpUrl] = Field(default=None, alias="AZURE_OPENAI_ENDPOINT")
    AZURE_OPENAI_API_KEY: Optional[str] = Field(default=None, alias="AZURE_OPENAI_API_KEY")
    AZURE_OPENAI_API_VERSION: str = Field(default="2024-02-01", alias="AZURE_OPENAI_API_VERSION")
    
    AZURE_OPENAI_EMBEDDING_DEPLOYMENT_NAME: Optional[str] = Field(default=None, alias="AZURE_OPENAI_EMBEDDING_DEPLOYMENT_NAME")
    AZURE_OPENAI_CHAT_DEPLOYMENT_NAME_DEFAULT: Optional[str] = Field(default=None, alias="AZURE_OPENAI_CHAT_DEPLOYMENT_NAME_DEFAULT")
    AZURE_OPENAI_CHAT_DEPLOYMENT_NAME_PREMIUM: Optional[str] = Field(default=None, alias="AZURE_OPENAI_CHAT_DEPLOYMENT_NAME_PREMIUM")
    DEFAULT_GENERATION_MODEL_NAME: str = Field(default="gpt-4o-mini", alias="DEFAULT_GENERATION_MODEL_NAME")
    
    # --- Standard OpenAI API Configuration (for DALL-E) ---
    OPENAI_API_KEY: Optional[str] = Field(default=None, alias="OPENAI_API_KEY")
    OPENAI_DALLE3_MODEL: str = Field(default="dall-e-3", alias="OPENAI_DALLE3_MODEL")
    ...
    AZURE_AI_SEARCH_ENDPOINT: Optional[AnyHttpUrl] = Field(default=None, alias="AZURE_AI_SEARCH_SERVICE_ENDPOINT")
    AZURE_AI_SEARCH_API_KEY: Optional[str] = Field(default=None, alias="AZURE_AI_SEARCH_API_KEY")
    AZURE_AI_SEARCH_INDEX_NAME: str = Field(default="rag-app-content-index", alias="AZURE_AI_SEARCH_INDEX_NAME")
    ...
    AZURE_STORAGE_CONTAINER_NAME_FOR_RAG_DOCS: str = Field(default="rag-documents", alias="AZURE_STORAGE_CONTAINER_NAME_FOR_RAG_DOCS")
    AZURE_STORAGE_CONTAINER_NAME_FOR_PUBLISHED_STORIES: str = Field(default="published-stories", alias="AZURE_STORAGE_CONTAINER_NAME_FOR_PUBLISHED_STORIES")
    AZURE_STORAGE_CONTAINER_NAME_FOR_GENERATED_IMAGES: str = Field(default="generated-images", alias="AZURE_STORAGE_CONTAINER_NAME_FOR_GENERATED_IMAGES")
    AZURE_STORAGE_CONTAINER_NAME_FOR_BLOG_MEDIA: str = Field(default="blogs", alias="AZURE_STORAGE_CONTAINER_NAME_FOR_BLOG_MEDIA")
    USE_AZURE_STORAGE_FOR_BLOG_MEDIA: bool = Field(default=False, alias="USE_AZURE_STORAGE_FOR_BLOG_MEDIA")
    ...
    # --- FIX: Temporarily restore these dictionaries and values ---
    # These are needed by the current SK plugin setup and will be removed later.
    ...
    AI_TEMPERATURE_SETTINGS: Dict[str, float] = {
        ...
        "rag_conversion": 0.3, "world_generation": 0.5, "world_elements_extraction": 0.3,
    ...
    AI_MAX_TOKEN_SETTINGS: Dict[str, int] = {
        ...
        "rag_conversion": 700, "world_generation": 3500, "world_elements_extraction": 8000,
    ...
    AI_TOP_P_SETTINGS: Dict[str, float] = {
        ...
        "rag_conversion": 0.9, "world_generation": 0.9, "world_elements_extraction": 0.8,
        "story_generation": 0.9, "test": 0.8
    }
    # --- END FIX ---
    
    RAG_RETRIEVAL_TOP_K: int = Field(default=3, alias="RAG_RETRIEVAL_TOP_K", gt=0, le=50)
    RAG_WRITING_ASSISTANT_TOP_K: int = Field(default=10, alias="RAG_WRITING_ASSISTANT_TOP_K", gt=0, le=50)
    RAG_CHAT_TOP_K: int = Field(default=7, alias="RAG_CHAT_TOP_K", gt=0, le=30)
    RAG_MIN_RELEVANCE_SCORE: float = Field(default=0.7, alias="RAG_MIN_RELEVANCE_SCORE", ge=0.0, le=1.0)
...
def log_application_settings():
    ...
    logger.info(f"Azure AI Search Endpoint: {settings.AZURE_AI_SEARCH_ENDPOINT}")
    logger.info(f"Key Vault Name: {settings.KEY_VAULT_NAME if settings.KEY_VAULT_NAME else 'Not Configured'}")
    
    logger.info("--- RAG & Model Settings ---")
    logger.info(f"RAG Index Name: {settings.AZURE_AI_SEARCH_INDEX_NAME}")
    logger.info(f"Embedding Deployment: {settings.AZURE_OPENAI_EMBEDDING_DEPLOYMENT_NAME}")
