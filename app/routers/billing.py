...
async def get_user_account(
    ...
    return ApiResponse.success_response(
        data=UserAccountResponse.model_validate(account, from_attributes=True)
    )
...
async def get_user_balance(
    ...
    try:
        if not current_user:
            ...
            return ApiResponse.success_response(
                data={"balance": 0.0, "currency": "Coins", "error": "No user session found"}
            )
        ...
        return ApiResponse.success_response(data={"balance": float(balance), "currency": "Coins"})
    except Exception as e:
        logger.error(f"Error getting balance for user {current_user.id if current_user else 'unknown'}: {e}")
        return ApiResponse.success_response(
            data={"balance": 0.0, "currency": "Coins", "error": str(e)}
        )
...
async def get_credit_packages(db: AsyncSession = Depends(get_db_session)):
    ...
    return ApiResponse.success_response(
        data=[CreditPackageResponse.model_validate(pkg, from_attributes=True) for pkg in packages]
    )
...
async def get_billing_dashboard(
    ...
    return ApiResponse.success_response(data=BillingDashboardResponse(
        account=account,
...
async def check_balance(
    ...
    return ApiResponse.success_response(data=BalanceCheckResponse(
...
async def get_recent_ai_costs(
    ...
    try:
        ...
        return ApiResponse.success_response(data=RecentAICostResponse(
...
async def get_last_ai_call(
    ...
    try:
        ...
        return ApiResponse.success_response(data=LastAICallResponse(
            last_call=last_call,
