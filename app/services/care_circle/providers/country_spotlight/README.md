# Country Spotlight Provider

## Purpose
A fun fact card about a different country from around the world.

## Runtime Contract
- Provider key: `country_spotlight`
...
- Provider config keys read: `countries`
...
## Output Shape
- Observed payload fields returned by the provider: `capital`, `country`, `flag`, `fun_fact`
...
- Move the large curated country pool into a UTF-8 JSON or YAML data file and add a load test so emoji and accented text do not drift.
- Add a lightweight content-review test that validates capitals and required fields when the curated pool changes.
