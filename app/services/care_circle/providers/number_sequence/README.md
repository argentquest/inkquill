# Number Sequence Provider

## Purpose
A gentle number pattern puzzle to keep your mind sharp.

## Runtime Contract
- Provider key: `number_sequence`
...
- Provider config keys read: `sequences`
...
## Output Shape
- Observed payload fields returned by the provider: `answer`, `hint`, `question`
...
- Move the large sequence bank into a data file and add UTF-8 validation; the current inline list already shows mojibake in several entries.
- Add focused tests that verify answers and hints stay aligned for representative puzzle rows.
