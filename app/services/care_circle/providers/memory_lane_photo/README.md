...
## Purpose
Implements the `memory_lane_photo` Care Circle provider and renders it through the shared base provider contract.

## Runtime Contract
- Provider key: `memory_lane_photo`
...
- Provider config keys read: `fallback_photos`
...
- Replace purely remote placeholder image sources with a reviewed local library or cached media set if consistency matters for production delivery.
- Add a small payload contract test that ensures every fallback photo entry includes both an image URL and description.
